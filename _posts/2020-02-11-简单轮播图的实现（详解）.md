...
![在这里插入图片描述](https://img-blog.csdnimg.cn/20200117195444651.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3FxXzM2NjIzNzk4,size_16,color_FFFFFF,t_70)这是因为我们已经把宽度固定成导入的`img`的宽度了，再设定浮动后一个`img`并不会浮动到前一个`img`的右边，因为那已经超过了我们设置的宽度。
